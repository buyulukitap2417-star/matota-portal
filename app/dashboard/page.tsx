 ...
 export default async function DashboardPage() {
   ...
   if (user) {
     ...
     const [studentRes, questionRes, submissionRes] = await Promise.all([
       ...
       // DÜZELTME: Bu sorgu artık sizin orijinal HTML dosyanızdaki mantıkla eşleşiyor.
       supabase.from('homework_submissions').select('*, homeworks!inner(teacher_id, is_completed)', { count: 'exact', head: true }).eq('homeworks.teacher_id', user.id).eq('homeworks.is_completed', false)
     ]);
